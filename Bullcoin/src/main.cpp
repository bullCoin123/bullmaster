...
#if defined(NDEBUG)
# error "Bullcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x4a4c7c1e6232cff7b151357e61040add93092e0e85c4374359650bbbe13446ed");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bullcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bullcoin Signed Message:\n";
...
{
    // Bullcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Bullcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; //changed to 1 day //3.5 * 24 * 60 * 60; // Bullcoin: 3.5 days
static const int64 nTargetSpacing = 10 * 60; //changed // 2.5 * 60; // Bullcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bullcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bullcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4a4c7c1e6232cff7b151357e61040add93092e0e85c4374359650bbbe13446ed");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is Bullcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1485975689;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2971132;
        ...
        assert(block.hashMerkleRoot == uint256("0xd8c0fff368ad133d33e5ba4505d8ca8f0ec122b7b7ca493b6b718ecad0465699"));

	if (false && block.GetHash() != hashGenesisBlock) //changed
        {
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Bullcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BullcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BullcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BullcoinMiner(CWallet *pwallet)
{
    printf("BullcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bullcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BullcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BullcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BullcoinMiner, pwallet));
}
